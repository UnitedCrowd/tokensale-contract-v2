...
contract UniversalCrowdsale is VestableToken
{
	...
	// @dev : review this
	uint256 public maxTokensToGiveInitiallyInEther = 5 ether;
	...
	function sell(address _investor, uint256 _amount, uint256 _etherValue) internal
	{
		...
		if(deposited[_investor] + _etherValue <= maxTokensToGiveInitiallyInEther) // this purchase is within limit
		{
			_transfer(this, _investor, _amount);
			emit Transfer(this, _investor, _amount);
		}
		else if(deposited[_investor] > maxTokensToGiveInitiallyInEther) // has already exceeded limit
		{
			tokensToGive[_investor] = tokensToGive[_investor].add(_amount);
		}
		else // this purchase crosses the limit
		{
			uint256 etherWorthToSendNow = ((maxTokensToGiveInitiallyInEther.sub(deposited[_investor])).div(_etherValue)).mul(_amount);
			tokensToGive[_investor] = tokensToGive[_investor].add(_amount.sub(etherWorthToSendNow));

			_transfer(this, _investor, etherWorthToSendNow);
			emit Transfer(this, _investor, etherWorthToSendNow);
		}
	}
	...
	function getMyRemainingTokens() public afterDeadline
	{
		require(tokensToGive[msg.sender] > 0);

		uint256 tokensToGiveValue = tokensToGive[msg.sender];
		tokensToGive[msg.sender] = 0;
		_transfer(this, msg.sender, tokensToGiveValue);
		emit Transfer(this, msg.sender, tokensToGiveValue);
	}

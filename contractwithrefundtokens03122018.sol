...
contract BasicToken is ERC20Basic, FreezableToken
{
	...
	string public constant name = "Universal";
	string public constant symbol = "UNV";
...
contract UniversalCrowdsale is VestableToken
{
	using SafeMath for uint;

	uint256 DEC = 10 ** uint256(decimals);
	uint256 public buyPrice = 1000000000000000 wei;
	...
	// @dev : review this
	uint256 public ICOdeadLine = 1533996414; // ICO end time - Sunday, 1 July 2018, 00:00:00.
	uint256 public REFUND_PERIOD = 14 days;

	mapping (address => uint256) public deposited;
	mapping (address => uint256) public whitelist;
	mapping (address => uint256) public tokensToGive;
	mapping (address => uint256) public timeOfLastPurchase;
	mapping (address => uint256) public etherOfLastPurchase;
	...
	function paymentManager(address sender, uint256 value) internal
	{
		...
		sell(sender, discountValue, value);
	...
	function sell(address _investor, uint256 _amount, uint256 _etherValue) internal
	{
		ICO.tokens = ICO.tokens.sub(_amount);

		if(tokensToGive[_investor] > 0)
		{
			_transfer(this, _investor, tokensToGive[_investor]);
		}

		tokensToGive[_investor] = _amount;
		timeOfLastPurchase[_investor] = now;
		etherOfLastPurchase[_investor] = _etherValue;
	}
	...
	function startCrowd(uint256 _tokens, uint _startDate, uint _endDate, uint8 _discount) public onlyOwner
	{
		...
		ICO = Ico (_tokens * DEC, _startDate, _startDate + _endDate * 1 days , _discount);
		stage = stage.add(1);
		unpauseInternal();
	}
	...
	function withdrawTokensORRefundEther() public // if called within REFUND_PERIOD, gives full refund. Else, gives tokens
	{
		require(tokensToGive[msg.sender] > 0);

		if(now - timeOfLastPurchase[msg.sender] > REFUND_PERIOD) // refund period end. send tokens
		{
			_transfer(this, msg.sender, tokensToGive[msg.sender]);
		}
		else // msg.sender wants a refund within REFUND_PERIOD.
		{
			uint256 ethr = etherOfLastPurchase[msg.sender];

			ICO.tokens = ICO.tokens.add(tokensToGive[msg.sender]);
			deposited[msg.sender] = deposited[msg.sender].sub(ethr);

			msg.sender.transfer(ethr);
		}
		tokensToGive[msg.sender] = 0;
	}
...
contract UniversalToken is UniversalCrowdsale
{
	using SafeMath for uint;
	...
	// contract is the bounty wallet itself
	...
	// @dev : change this
	address public bountyManager = 0x1FDAF6AB6CeFa272e07B1B9e8C2Bc1b6Bb5b5694;

	uint256 public totalBountyTokensOwed;
	uint256 public maxBountyTokens;
	mapping (address => uint256) public bountyTokensOwed;

	event LogBountyAdded (address hunter, uint256 amount);
	event LogBountyAwarded (address hunter, uint256 amount);

	constructor() public
	{
		// @dev : revive this
		balances[this] = (totalSupply.mul(65)).div(100);                    // Send 60% of tokens to smart contract wallet      420,000,000 LOGIQ
		...
		maxBountyTokens = (totalSupply.mul(5)).div(100);             		// Send 5%  of tokens to bounty wallet              35,000,000 LOGIQ
		balances[this] = maxBountyTokens;
	...
	modifier onlyBountyManagerOROwner()
	{
		require(msg.sender == owner || msg.sender == bountyManager);
		_;
	}

	function makeNewBountyManager(address newBountyManager) public onlyOwner
	{
		require(newBountyManager != address(0));
	...
	function addBounty(address[] _hunters, uint256[] _tokenOwed) external onlyBountyManagerOROwner
	{
		uint huneterArrayLength = _hunters.length;
		uint tokenArrayLength = _tokenOwed.length;

		assert(huneterArrayLength == tokenArrayLength);

		for (uint256 i = 0; i < tokenArrayLength; i++)
		{
			bountyTokensOwed[_hunters[i]] = bountyTokensOwed[_hunters[i]].add(_tokenOwed[i]);
			totalBountyTokensOwed = totalBountyTokensOwed.add(_tokenOwed[i]);
			emit LogBountyAdded(_hunters[i], _tokenOwed[i]);
		}
		require (totalBountyTokensOwed <= maxBountyTokens);
	}

	function claimBounty() external
	{
		require (bountyTokensOwed[msg.sender] > 0);

		uint256 toSend = bountyTokensOwed[msg.sender];
		bountyTokensOwed[msg.sender] = 0;

		_transfer(this, msg.sender, toSend);
		emit LogBountyAwarded(msg.sender, toSend);
	}
